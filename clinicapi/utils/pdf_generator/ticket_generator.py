...
CURRENT_PLATFORM = pathlib.sys.platform
...
BINARY_PATH = CURRENT_PATH.joinpath(platform[CURRENT_PLATFORM])
...
def create_pdf(template, vars, css=[]):
	...
	config = pdfkit.configuration(wkhtmltopdf=str(BINARY_PATH))
	try:
		pdfkit.from_string(
			html,
			output_dir,
